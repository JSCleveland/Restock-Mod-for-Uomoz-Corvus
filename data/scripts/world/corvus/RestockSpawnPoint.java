	...
	@Override
		...
		if (aCargo.isFreeTransfer() == false){// only if the station is not the player station
			...
			if (station.getName().toString().equals("Trade Station")){
			    restockShips(aMothFleet,dShips);
			    restockShips(aMothFleet,mShips);
			    restockShips(aMothFleet,aspShips);
			    restockFighters(aMothFleet,dWings);
			    restockFighters(aMothFleet,mWings);
			    restockFighters(aMothFleet,aspWings);
